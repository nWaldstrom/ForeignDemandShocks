...
#from ForeignEcon.Foreign_steady_state import find_foreign_ss
...
#model.path.betaF[:,:] = model.ss.betaF
utils.Create_shock('ZF', model, 0.01, absval=False)

model.transition_path(do_print=True)


#%%
model.show_IRFs(['C_s','piF_s', 'rF_s'],T_max=40,do_shocks=False,do_targets=False, abs_diff=['piF_s'])


#%%

model_alt = model.copy(name='alt')
model_alt.par.sigma = 0.5
utils.Create_shock('ZF', model_alt, 0.01, absval=False)

#utils.Create_shock('deps', model_alt, 0.0003, absval=False)
model_alt.transition_path(do_print=True)


models = [model,model_alt]
labels = ['eis=0.5', 'deps']
paths = ['C_s','piF_s', 'rF_s']
model.compare_IRFs(models,labels,paths,T_max=40,do_shocks=False,do_targets=False, abs_diff=['piF_s'])


#%%


# from ForeignEcon.ForeignEconSolve import compute_jac 
import ForeignEcon.ForeignEconSolve as ForeignEconSolve


ForeignEconSolve.compute_jac(model)
...
utils.Create_shock('betaF', model, -0.0001, absval=False)
# ForeignEconSolve.find_transition_path(model, do_print=True)
ForeignEconSolve.find_foreign_transition_path(model, do_print=True)
...
abs_diff = ['iF_s']
#facs = {'r':100,'rk':100}
model.show_IRFs(paths,abs_diff=abs_diff,ncols=3,T_max=100, do_targets=False)
